...
def save_capture(frame, dirs: dict, idx: int, cfg_camera: dict) -> dict:
    dt_name   = datetime.now().strftime("%Y%m%d_%H%M%S")  # 모든 파일 공통 타임스탬프
...
def process_one_frame(
    ...
    # 저장된 파일명에서 타임스탬프 추출 → results 파일명 통일
                ...
                try:
                    while True:
                        ...
                        if cmd == "QUIT":
                            ...
                            _vi = {
                                "status":      _nf.get("status", payload.get("status", "-")),
                                "frame_idx":   frame_idx,
                                "timestamp":   _ts,
                                "capture_ms":  _cs.get("capture_ms", 0),
                                "valid_ratio": _cs.get("valid_ratio", 0),
                                "z_min":       _cs.get("z_min", 0),
                                "z_max":       _cs.get("z_max", 0),
                                "det_ms":      _nf.get("det_ms", 0),
                                "icp_ms":      _nf.get("icp_ms", 0),
                                "picks":       _nf.get("picks", []),
                            }
                            if _ov is not None:
